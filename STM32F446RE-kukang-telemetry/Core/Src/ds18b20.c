...
static uint8_t OW_Reset(GPIO_TypeDef *GPIOx, uint16_t GPIO_Pin) {
  ...
  // Polling kehadiran (Presence Pulse) untuk antisipasi timing sensor
  // clone/kapasitansi kabel Karena sensor merespons sekitar ~130us, kita tunggu
  // hingga max 200us
  uint16_t wait_time = 0;
  while (HAL_GPIO_ReadPin(GPIOx, GPIO_Pin) == GPIO_PIN_SET && wait_time < 200) {
    delay_us(2);
    wait_time += 2;
  }

  if (wait_time < 200) {
    response = 1; // Terdeteksi sinyal ditarik low oleh slave

    // Tunggu hingga sinyal dikembalikan menjadi high oleh pull-up
    uint16_t presence_time = 0;
  ...
  delay_us(200); // Waktu recovery sebelum pengiriman perintah berikutnya
  return response;
}
...
static void OW_WriteBit(GPIO_TypeDef *GPIOx, uint16_t GPIO_Pin, uint8_t bit) {
  if (bit) {
    HAL_GPIO_WritePin(GPIOx, GPIO_Pin, GPIO_PIN_RESET);
    delay_us(2); // Tarik LOW sebentar (1-5 us)
    HAL_GPIO_WritePin(GPIOx, GPIO_Pin, GPIO_PIN_SET);
    delay_us(60); // Biarkan HIGH selama sisa slot
  } else {
    HAL_GPIO_WritePin(GPIOx, GPIO_Pin, GPIO_PIN_RESET);
    delay_us(60); // Tarik LOW sepanjang slot
    HAL_GPIO_WritePin(GPIOx, GPIO_Pin, GPIO_PIN_SET);
    delay_us(10); // Berikan waktu recovery (t_REC) agar tegangan naik sempurna
                  // ke 3.3V
  }
}
...
static uint8_t OW_ReadBit(GPIO_TypeDef *GPIOx, uint16_t GPIO_Pin) {
  uint8_t bit = 0;
  HAL_GPIO_WritePin(GPIOx, GPIO_Pin, GPIO_PIN_RESET);
  delay_us(2); // Tarik LOW untuk inisiasi slot baca
  HAL_GPIO_WritePin(GPIOx, GPIO_Pin, GPIO_PIN_SET);
  delay_us(8); // Tunggu 8 us (total ~10-11 us dari falling edge, tepat di
               // tengah valid window 15 us)
  if (HAL_GPIO_ReadPin(GPIOx, GPIO_Pin) == GPIO_PIN_SET) {
    bit = 1;
  }
  delay_us(50); // Selesaikan sisa durasi slot (total >= 60 us)
  return bit;
}
...
static uint8_t OW_Search(GPIO_TypeDef *GPIOx, uint16_t GPIO_Pin,
                         uint8_t *newAddr) {
...
uint8_t DS18B20_Search(GPIO_TypeDef *GPIOx, uint16_t GPIO_Pin,
                       DS18B20_Device *devices, uint8_t max_devices) {
  uint8_t num_found = 0;
  LastDiscrepancy = 0;
  LastDeviceFlag = 0;
  LastFamilyDiscrepancy = 0;

  while (num_found < max_devices &&
         OW_Search(GPIOx, GPIO_Pin, devices[num_found].rom_code)) {
...
float DS18B20_ReadTemp(GPIO_TypeDef *GPIOx, uint16_t GPIO_Pin,
                       uint8_t *rom_code) {
  if (!OW_Reset(GPIOx, GPIO_Pin))
    return -999.0f;
